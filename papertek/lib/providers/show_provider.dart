...
// Compatibility read provider. Lifecycle mutation belongs to ShowSessionNotifier.
final showSessionProvider = NotifierProvider<ShowSessionNotifier, ShowSession?>(
  ShowSessionNotifier.new,
);
final databaseProvider = Provider<AppDatabase?>((ref) {
  ref.listen<ShowSession?>(showSessionProvider, (_, __) {
    ref.read(designerModeProvider.notifier).state = false;
    ref.read(activeReportTemplateIdProvider.notifier).state = null;
    ref.read(activeReportTemplateDirtyProvider.notifier).state = false;
  });
  return ref.watch(showSessionProvider)?.database;
});
final currentShowPathProvider = Provider<String?>(
  (ref) => ref.watch(showSessionProvider)?.path,
);

/// Stateless service responsible for file-system operations like creating,
/// opening, and saving .papertek (SQLite) files.
final showFileServiceProvider = Provider<ShowFileService>(
  (ref) => ShowFileService(),
);
...
final trackedWriteProvider = Provider<TrackedWriteRepository?>((ref) {
...
final revisionRepoProvider = Provider.autoDispose<RevisionRepository?>((ref) {
  final db = ref.watch(databaseProvider);
...
final pendingFixtureIdsProvider = StreamProvider.autoDispose<Set<int>>((ref) {
...
final conflictFixtureIdsProvider = StreamProvider.autoDispose<Set<int>>((ref) {
  final repo = ref.watch(revisionRepoProvider);
  if (repo == null) return Stream.value({});
...
final showMetaRepoProvider = Provider.autoDispose<ShowMetaRepository?>((ref) {
  final db = ref.watch(databaseProvider);
...
final currentShowMetaProvider = StreamProvider.autoDispose<ShowMetaData?>((
  ref,
) {
  final db = ref.watch(databaseProvider);
...
final positionRepoProvider = Provider.autoDispose<PositionRepository?>((ref) {
  final db = ref.watch(databaseProvider);
  final tracked = ref.watch(trackedWriteProvider);
  if (db == null || tracked == null) return null;
...
final positionGroupsProvider = StreamProvider.autoDispose<List<PositionGroup>>((
  ref,
) {
  final repo = ref.watch(positionRepoProvider);
  if (repo == null) return Stream.value([]);
...
final fixtureTypeRepoProvider = Provider.autoDispose<FixtureTypeRepository?>((
  ref,
) {
  final db = ref.watch(databaseProvider);
  final tracked = ref.watch(trackedWriteProvider);
...
final fixtureTypesProvider = StreamProvider.autoDispose<List<FixtureType>>((
  ref,
) {
...
final addressesProvider = StreamProvider.autoDispose<List<AddressesData>>((
  ref,
) {
  final repo = ref.watch(venueRepoProvider);
  if (repo == null) return Stream.value([]);
...
final fixtureRowsProvider = StreamProvider.autoDispose<List<FixtureRow>>((ref) {
...
final customFieldRepoProvider = Provider.autoDispose<CustomFieldRepository?>((
  ref,
) {
  final db = ref.watch(databaseProvider);
  final tracked = ref.watch(trackedWriteProvider);
...
final customFieldsProvider = StreamProvider.autoDispose<List<CustomField>>((
  ref,
) {
...
final reportTemplateRepoProvider =
    Provider.autoDispose<ReportTemplateRepository?>((ref) {
      final db = ref.watch(databaseProvider);
...
final operationalRepoProvider = Provider.autoDispose<OperationalRepository?>((
  ref,
) {
  final db = ref.watch(databaseProvider);
  if (db == null) return null;
...
final unresolvedMaintenanceProvider =
    StreamProvider.autoDispose<List<MaintenanceLogData>>((ref) {
      final db = ref.watch(databaseProvider);
      if (db == null) return Stream.value([]);
      return (db.select(
        db.maintenanceLog,
      )..where((t) => t.resolved.equals(0))).watch();
    });

/// Repository for user-editable field display names (e.g. "Instrument" → "Fixture Type").
final fieldNameRepositoryProvider = Provider.autoDispose<FieldNameRepository?>((
  ref,
) {
  final db = ref.watch(databaseProvider);
  if (db == null) return null;
